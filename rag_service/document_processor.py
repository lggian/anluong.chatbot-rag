...
from app.processors.chunk_record import ChunkRecord
...
async def preview_document(file_path: str, chunk_size: int = 1000, chunk_overlap: int = 200) -> PreviewResult:
    ...
    # 1. Tạo file tạm nhưng đóng nó ngay lập tức
    fd, temp_path = tempfile.mkstemp(suffix=ext)
    os.close(fd) # Đóng file descriptor ngay để giải phóng quyền truy cập

    try:
        # 2. Bây giờ MinIO có thể toàn quyền ghi vào temp_path
        minio_client = get_minio_client()
        
        clean_file_path = file_path.replace('\\', '/')
        minio_client.fget_object(
            bucket_name=settings.MINIO_BUCKET_NAME,
            object_name=clean_file_path, # Đây là tên file trên MinIO (Key)
            file_path=temp_path
        )
        
        # ... (tiếp tục đoạn code xử lý loader và split text)
        ...
        # 3. Sau khi xử lý xong (ở cuối hàm), hãy nhớ xóa file tạm
